...
def show_message(*args):
    from ubuntutweak0.ui.dialogs import ErrorDialog
    message = _('The Ubuntu Tweak daemon didn\'t start correctly. This means that some '
            'advanced features may not work.\n'
            'If you want to help developers debugging, try to run "<b>sudo /usr/share/ubuntu-tweak-0/ubuntu-tweak-daemon</b>" in a terminal.')
...
class DbusProxy:
    INTERFACE = "com.ubuntu_tweak.daemonold"

    try:
        __system_bus = dbus.SystemBus()
        __object = __system_bus.get_object('com.ubuntu_tweak.daemonold', '/com/ubuntu_tweak/daemonold')
    except Exception, e:
        __object = None
