...
gchar *startup_image="pixmaps/startup.png";
gchar *personality_image="pixmaps/desktop.png";
gchar *other_image="pixmaps/apperance.png";
gchar *security_image="pixmaps/security.png";
...
/*创建程序主窗体，分别设定好窗口标题，窗口默认大小，窗口默认打开位置（屏幕中心）*/
	...
	gtk_window_set_title(GTK_WINDOW(window),_("CODENAME:Tualatin V0.2"));
...
/*将LOGO插入head区域*/
	headline=gtk_image_new_from_file(banner_left);
	gtk_widget_show(headline);
	gtk_box_pack_start(GTK_BOX(hbox_head),headline,FALSE,FALSE,0);

	headline_2=gtk_image_new_from_file(banner_right);
	gtk_widget_show(headline_2);
	gtk_box_pack_end(GTK_BOX(hbox_head),headline_2,FALSE,FALSE,0);

/*正文默认的欢迎字样*/

	label_welcome=gtk_label_new(_("Welcome! \nThank you for using Ubuntu Tweak, But now it isn't finished, and it isn't Alpha Version yet.\n So please wait for some days.\n Hahahaha...."));
